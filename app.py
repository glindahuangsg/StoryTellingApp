...
=====================================================================================
 ...
 WHAT THIS APP DOES (pipeline overview)
   1. IMAGE INPUT     : The user uploads a picture through the Streamlit UI.
   ...
   4. TEXT-TO-SPEECH  : The story text is converted into an MP3 audio clip
                         (gTTS) so kids can listen to it read aloud, as required
...
def generate_caption(image: Image.Image, captioner) -> str:
    ...
    Args:
        image: the PIL image uploaded by the user.
...
def text_to_speech(story_text: str) -> io.BytesIO:
    """
    Convert the generated story text into spoken audio (MP3) using gTTS
    (Google Text-to-Speech), fulfilling the assignment's Text-to-Speech
    Conversion requirement.

    gTTS was chosen because it is lightweight, needs no extra system
    dependencies or GPU, and installs cleanly on Streamlit Cloud's free tier,
    which matters a lot for the Deployment grading criterion.

    Args:
        story_text: the story to convert to speech.

    Returns:
        An in-memory MP3 audio file (BytesIO), ready to play/download in Streamlit.
    """
    speech = gTTS(text=story_text, lang="en", slow=False)
...
def render_header() -> None:
    ...
    st.title("📖✨ Story Time! ✨📖")
    st.markdown(
        "**Upload a picture and watch it turn into a magical story you can listen to!**"
    )
    st.markdown("---")
...
def run_storytelling_pipeline(image: Image.Image):
    """
...
def main() -> None:
    ...
    if uploaded_file is None:
        st.info("👆 Upload a picture above to get started!")
        return

    # Show the uploaded picture to the user right away, before any AI runs.
